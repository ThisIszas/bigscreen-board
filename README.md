...
## 安装
    ...
    npm install @zl/bigscreen-board echarts echarts-liquidfill
    # 或
    pnpm add @zl/bigscreen-board echarts echarts-liquidfill
...
## 快速开始
    ...
    import { ScaleScreen, BoardPanel, ChartWidget, useBoardPolling } from '@zl/bigscreen-board';
    import '@zl/bigscreen-board/style.css'; // 组件样式 + 数字字体(必须引入一次)
...
样式文件必须引入一次（<code>import '@zl/bigscreen-board/style.css'</code>）：其中包含面板/适配容器
